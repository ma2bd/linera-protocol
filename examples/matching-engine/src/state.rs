...
impl LevelView {
    ...
    pub async fn modify_order_level(
        ...
        let new_amount = match reduce_quantity {
        ...
        let corr_reduce_quantity = state_order.amount.try_sub(new_amount).unwrap();
        state_order.amount = new_amount;
        self.remove_zero_orders_from_level().await;
        Some((corr_reduce_quantity, new_amount == Amount::ZERO))
    }
}
